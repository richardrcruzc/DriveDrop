...
namespace DriveDrop.Api.Application.IntegrationEvents.EventHandling
{
    public class UserCheckoutAcceptedIntegrationEventHandler : IIntegrationEventHandler<UserCheckoutAcceptedIntegrationEvent>
    {
        ...
        private readonly IShippingIntegrationEventService _orderingIntegrationEventService;

        public UserCheckoutAcceptedIntegrationEventHandler(IMediator mediator,
            ILoggerFactory logger, IShippingIntegrationEventService orderingIntegrationEventService)
        {
        ...
        public async Task Handle(UserCheckoutAcceptedIntegrationEvent eventMsg)
        {
            ...
            var orderStartedIntegrationEvent = new ShippingStartedIntegrationEvent(eventMsg.UserId);
            await _orderingIntegrationEventService.PublishThroughEventBusAsync(orderStartedIntegrationEvent);

            if (eventMsg.RequestId != Guid.Empty)
            {
                var createOrderCommand = new CreateShippingCommand(eventMsg.Basket.Items, eventMsg.UserId, eventMsg.City, eventMsg.Street, 
                ...
                var requestCreateOrder = new IdentifiedCommand<CreateShippingCommand, bool>(createOrderCommand, eventMsg.RequestId);
